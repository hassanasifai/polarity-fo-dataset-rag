...
- **What this produces:** 50 validated Family Office records (`data/processed/family_offices_validated.xlsx`) with a 15-sheet audit trail (sources, field evidence, validation results, Apify/Firecrawl evidence, contact info, news signals, LinkedIn enrichment, Google Places, validation chain snippets, data dictionary).
- **Headline metrics:** 50 / 50 records accepted, all `confidence=high`, 112 source URLs (≥2 per record), 86 tests passing at 90.24 % coverage, ruff clean, `pip_audit` clean.
...
flowchart TD
    ...
    K --> M[Contact promotion<br/>promote_contacts.py]
    L --> N[Chain snippet extraction<br/>chain_snippets.py]
    K --> O[News promotion<br/>promote_news.py]
    ...
    Q --> R[Final XLSX<br/>15 sheets · 50 records · 48 cols]
    Q --> S[Final CSV + JSON]
...
| `promote_news.py` | Filters Google News evidence (≥2025-01-01, name-token match, no directory sources) and populates `recent_activity`. Records with no surviving signal get an explicit "no recent public signal" uncertainty note. | `python -m fo_dataset_pipeline.promote_news` |
| `enrich_columns.py` | Adds sample-workbook-parity columns: `data_validation_period`, `family_office_domain`, `url_quality`, plus joined email-validation columns. | `python -m fo_dataset_pipeline.enrich_columns` |
| `refresh_xlsx.py` | Rebuilds the 15-sheet workbook from on-disk CSVs **without** rerunning `cli validate` (which would wipe promoted fields by reading the seed). | `python -m fo_dataset_pipeline.refresh_xlsx` |
...
# Stage 7: rebuild the workbook from current CSV state
...
| File | What it proves |
|---|---|
| `family_offices_validated.xlsx` | The deliverable. 15 sheets, 50 records, 48 columns. |
...
| `field_evidence.csv` | 476 rows. Every non-empty field claim mapped to a source URL + claim type + confidence. |
...
- **Believed vs verified** — every promoted contact carries a `*_confidence` label (`corporate_public_listed` / `linkedin_scraped_match` / `linked_from_official_site`) and a `*_evidence_url`.
...
- **Principal-level contact intelligence is absent by design.** No row contains principal-specific email, phone, or LinkedIn — the public sources do not link a personal contact channel to a named principal role with sufficient confidence.
- **Recent activity coverage is 21 of 50.** Small or private offices without 2025+ press coverage are tagged in `uncertainty_notes` rather than padded with weak matches.
...
| xlsx audit | 0 issues, 29 checks passed |
